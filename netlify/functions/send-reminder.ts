...
export const handler: Handler = async event => {
  ...
  try {
    ...
    if (input.f29_period_id) {
      const { data: period } = await context.supabase.from('f29_periods').select('id,client_id,year,month,amount,due_day,tax_paid,clients(legal_name)').eq('id', input.f29_period_id).single();
