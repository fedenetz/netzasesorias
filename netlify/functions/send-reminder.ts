...
import { authenticate, functionError, json, parseBody, renderTemplate, resolveEmployeeEmail, sanitizeHtml, sendWithResend, validateEmails } from './_shared';
...
export const handler: Handler = async event => {
  ...
  try {
    ...
    if (input.f29_period_id) {
      ...
      responsibleName = period.responsible_name || '';
      title = 'Recordatorio de pago F29';
      periodOrConcept = `${variables.month_name} ${variables.year}`;
      summary = [{ label: 'Cliente', value: variables.client_name }, { label: 'Período', value: periodOrConcept }, { label: 'Monto pendiente', value: variables.amount }, { label: 'Vencimiento', value: period.due_day ? `Día ${period.due_day}` : 'Por confirmar' }];
    } else {
      const { data: item } = await context.supabase.from('billing_items').select('*,clients(legal_name),payment_links(external_url,status,created_at)').eq('id', input.billing_item_id).single();
      ...
      const links: Array<{ external_url?: string; status?: string; created_at?: string }> = Array.isArray(item.payment_links) ? item.payment_links : [];
      paymentLink = String(links.filter(link => link.status === 'active').sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)))[0]?.external_url ?? '');
      variables = { client_name: clientName(item.clients), billing_amount: money(item.amount), due_date: item.due_date || 'Sin fecha', billing_status: item.status, service_period: item.description, payment_link: paymentLink, firm_name: process.env.FIRM_NAME || 'Netz Asesorías' };
      periodOrConcept = item.description;
      summary = [{ label: 'Cliente', value: variables.client_name }, { label: 'Servicio / concepto', value: item.description }, { label: 'Monto pendiente', value: variables.billing_amount }, { label: 'Fecha de vencimiento', value: variables.due_date }];
    }
    ...
    const html = sanitizeHtml(renderNetzEmail({ title, eyebrow: 'Comunicación de cobranza', clientName: variables.client_name, periodOrConcept, bodyHtml, summary, responsibleName: responsibleName || undefined, replyTo, logoUrl, paymentLink }));
    ...
    logId = log.id;
    const providerId = await sendWithResend({ from, to, cc, subject, html, attachments: [], reply_to: replyTo }, log.id);
