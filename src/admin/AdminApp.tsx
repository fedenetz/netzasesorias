...
function ClientsIndexV3({ rows, go, reload }: { rows: ClientRow[]; go: (s: Screen, c?: ClientRow) => void; reload: () => Promise<void> }) {
  const [query,setQuery]=useState(''); const [responsible,setResponsible]=useState('Todos'); const [driveFilter,setDriveFilter]=useState('Todos'); const [f29Filter,setF29Filter]=useState('Todos'); const [billingFilter,setBillingFilter]=useState('Todos'); const [editing,setEditing]=useState<ClientRow|'new'|null>(null); const [scanning,setScanning]=useState<string|null>(null); const [error,setError]=useState('');
  const responsibles=[...new Set(rows.map(row=>row.accountant))].sort();
  ...
  const filtered=rows.filter(row=>`${row.rut} ${row.name} ${row.accountingCode??''}`.toLowerCase().includes(query.toLowerCase())&&(responsible==='Todos'||row.accountant===responsible)&&(driveFilter==='Todos'||(driveFilter==='Con Drive'?Boolean(row.driveFolderId):!row.driveFolderId))&&(f29Filter==='Todos'||(f29Filter==='Sin estado'?!row.statusCode:row.statusCode===f29Filter))&&(billingFilter==='Todos'||row.billingStatus===billingFilter));
  const openDrive=(event:React.MouseEvent,row:ClientRow)=>{event.stopPropagation();if(row.driveFolderId)window.open(`https://drive.google.com/drive/folders/${row.driveFolderId}`,'_blank','noopener,noreferrer');};
  const refreshDocs=async(event:React.MouseEvent,row:ClientRow)=>{event.stopPropagation();setScanning(row.id);setError('');try{await scanClientDrive(row.id);await reload();}catch(reason){setError(reason instanceof Error?reason.message:'No fue posible actualizar Drive.');}finally{setScanning(null);}};
  return <div className="control-content"><PageHeader eyebrow="Base maestra" title="Clientes" description="Cartera activa, cumplimiento y estado de facturación." actions={<button className="button-dark" onClick={()=>setEditing('new')}><Plus size={16}/> Nuevo cliente</button>}/>{error&&<div className="control-data-state is-error"><AlertTriangle size={16}/>{error}</div>}<section className="operations-card"><div className="table-toolbar clients-toolbar"><div className="toolbar-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar RUT, Conta o razón social…"/></div><select value={responsible} onChange={event=>setResponsible(event.target.value)}><option>Todos</option>{responsibles.map(name=><option key={name}>{name}</option>)}</select><select value={f29Filter} onChange={event=>setF29Filter(event.target.value)}><option>Todos</option><option>Sin estado</option>{Object.entries(F29_STATUS_LABELS).map(([code,label])=><option value={code} key={code}>{label}</option>)}</select><select value={billingFilter} onChange={event=>setBillingFilter(event.target.value)}><option>Todos</option><option value="not_applicable">Facturación: No aplica</option><option value="pending">Facturación: Pendiente</option><option value="sent">Facturación: Enviada</option><option value="paid">Facturación: Pagada</option><option value="overdue">Facturación: Vencida</option></select><select value={driveFilter} onChange={event=>setDriveFilter(event.target.value)}><option>Todos</option><option>Con Drive</option><option>Sin Drive</option></select></div><div className="table-scroll"><table className="ops-table clients-table clients-operations-table"><thead><tr><th>RUT</th><th>Conta</th><th>Razón social</th><th>Contador asignado</th><th>Drive</th><th>F29 actual</th><th>Facturación</th><th>Docs</th><th>Última actualización</th><th/></tr></thead><tbody>{filtered.map(row=><tr key={row.id} onClick={()=>go('client',row)}><td><strong>{row.rut}</strong></td><td><strong>{row.accountingCode||'—'}</strong></td><td><strong>{row.name}</strong></td><td><span className={`assigned responsible-client ${color(row.accountant)}`}><EmptyAvatar initials={row.initials}/>{row.accountant}</span></td><td><button className="drive-button" disabled={!row.driveFolderId} onClick={event=>openDrive(event,row)}><FolderOpen size={15}/>{row.driveFolderId?'Abrir carpeta':'Sin carpeta'}</button></td><td><Pill value={row.statusLabel}>{row.statusLabel}</Pill></td><td><StatusBadge status={row.billingStatus}/></td><td>{row.documents||<button className="quick-refresh-docs" disabled={!row.driveFolderId||scanning===row.id} onClick={event=>void refreshDocs(event,row)}><RefreshCw size={13}/>{scanning===row.id?'Actualizando':'Recargar'}</button>}</td><td>{row.updated}</td><td><button className="icon-button compact" aria-label={`Editar ${row.name}`} onClick={event=>{event.stopPropagation();setEditing(row);}}><Settings size={14}/></button></td></tr>)}</tbody></table></div><footer className="table-footer"><span>{filtered.length} clientes</span><span>{rows.filter(row=>row.driveFolderId).length} con carpeta Drive</span></footer></section>{editing&&<ClientEditor client={editing==='new'?undefined:editing} onClose={()=>setEditing(null)} onSaved={async()=>{setEditing(null);await reload();}}/>}</div>;
}
