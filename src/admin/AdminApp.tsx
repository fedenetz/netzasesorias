...
import { addClientObservation, classifyDocument, loadClientActivity, loadClientDocuments, loadClientObservations, saveClient, scanClientDrive } from './client-api';
...
function DocumentsPanel({ client, documents, refresh }: { client: ClientRow; documents: ClientDocument[]; refresh: () => Promise<void> }) {
  ...
  const scan = async () => { setScanning(true); setMessage(''); try { const result = await scanClientDrive(client.id); setMessage(`${result.files_found} archivos encontrados · ${result.new_files} nuevos · ${result.updated_files} actualizados`); await refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Error al escanear Drive.'); } finally { setScanning(false); } };
