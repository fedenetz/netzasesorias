...
import { canEditClient, documentGroup, generateHistoryMonths, isRecentRelevantDocument, type DocumentGroup } from './operational-utils';
...
type F22Filter = 'bce_pending' | 'f22_pending' | 'f22_sent' | 'dj_pending';
type F22SortKey = 'rut' | 'client' | 'regime' | 'bceDate' | 'bce' | 'f22Ready' | 'f22Sent' | 'dj1948' | 'djSent' | 'dj1949' | 'provisional' | 'utility' | 'dividends' | 'observations' | 'responsible' | 'saved';
const f22FilterOptions = [
  { value: 'bce_pending', label: 'BCE pendiente' }, { value: 'f22_pending', label: 'F22 pendiente' },
  { value: 'f22_sent', label: 'F22 enviado' }, { value: 'dj_pending', label: 'DJ pendiente' },
];
const isF22Alert = (row: F22Row) => Boolean(row.observation.trim()) || /error|vencid|posterg|revisi/i.test(row.reviewStatus);
...
function F22DashboardLive({ initialTaxYear, openClient }: { initialTaxYear: number; openClient: (clientId: string) => void }) {
  ...
  const [query, setQuery] = useState('');
  const [filters, setFilters] = useState<F22Filter[]>([]);
  const [onlyAlerts, setOnlyAlerts] = useState(false);
  const [sort, setSort] = useState<SortState<F22SortKey>>({ key: 'rut', direction: 'asc' });
  const [page, setPage] = useState(1);
  ...
  useEffect(() => setPage(1), [query, filters, onlyAlerts, taxYear]);
  ...
  const filtered = useMemo(() => rows.filter(row => {
    const categoryMatch = !filters.length || filters.some(filter => (filter === 'bce_pending' && !row.bceDate) || (filter === 'f22_pending' && !row.f22Sent) || (filter === 'f22_sent' && row.f22Sent) || (filter === 'dj_pending' && row.dj1948 === true && !row.dj1948Sent));
    return `${row.rut} ${row.name}`.toLowerCase().includes(query.toLowerCase()) && categoryMatch && (!onlyAlerts || isF22Alert(row));
  }).sort((a, b) => {
    const values: Record<F22SortKey, (row: F22Row) => string | number | boolean | null> = {
      rut: row => row.rut, client: row => row.name, regime: row => row.taxRegime, bceDate: row => row.bceDate,
      bce: row => Boolean(row.bceDate), f22Ready: row => Boolean(row.f22Ready), f22Sent: row => row.f22Sent,
      dj1948: row => Boolean(row.dj1948), djSent: row => Boolean(row.dj1948Sent), dj1949: row => Boolean(row.dj1949),
      provisional: row => Boolean(row.provisional), utility: row => row.utilityLossAmount ?? row.utilityLossText,
      dividends: row => row.dividendsAmount ?? row.dividendsText, observations: row => row.observation,
      responsible: row => row.responsibleName, saved: row => saving[row.id] ?? '',
    };
    return compareSortValues(values[sort.key](a), values[sort.key](b), sort.direction) || a.rut.localeCompare(b.rut, 'es', { numeric: true });
  }), [rows, query, filters, onlyAlerts, sort, saving]);
  const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize));
  const shown = filtered.slice((page - 1) * pageSize, page * pageSize);
...
function ClientsIndexV3({ rows, go, reload, role, userId }: { rows: ClientRow[]; go: (s: Screen, c?: ClientRow) => void; reload: () => Promise<void>; role: 'admin'|'accountant'; userId?: string }) {
  type ClientSortKey = 'client'|'responsible'|'f29'|'billing'|'documents'|'updated';
  const [query,setQuery]=useState(''); const [responsibleFilters,setResponsibleFilters]=useState<string[]>([]); const [documentFilters,setDocumentFilters]=useState<string[]>([]); const [f29Filters,setF29Filters]=useState<string[]>([]); const [billingFilters,setBillingFilters]=useState<string[]>([]); const [onlyAlerts,setOnlyAlerts]=useState(false); const [sort,setSort]=useState<SortState<ClientSortKey>>({key:'client',direction:'asc'}); const [filtersOpen,setFiltersOpen]=useState(false); const [editing,setEditing]=useState<ClientRow|'new'|null>(null); const [inspectorId,setInspectorId]=useState<string|null>(null); const [scanning,setScanning]=useState<string|null>(null); const [scanResults,setScanResults]=useState<Record<string,{count:number;at:string}>>({}); const [error,setError]=useState('');
  const responsibles=[...new Set(rows.map(row=>row.accountant))].sort();
  ...
  const filtered=rows.filter(row=>`${row.rut} ${row.name} ${row.accountingCode??''}`.toLowerCase().includes(query.toLowerCase())&&(!responsibleFilters.length||responsibleFilters.includes(row.accountant))&&(!documentFilters.length||documentFilters.some(filter=>filter==='with'?countFor(row)>0:filter==='unreviewed'?!scanAt(row):countFor(row)===0))&&(!f29Filters.length||f29Filters.some(filter=>filter==='missing'?!row.statusCode:row.statusCode===filter))&&(!billingFilters.length||billingFilters.includes(row.billingStatus))&&(!onlyAlerts||Boolean(row.observation.trim())||['B','E','G','H'].includes(row.statusCode??'')||row.billingStatus==='overdue')).sort((a,b)=>{const values:Record<ClientSortKey,(row:ClientRow)=>string|number|null>={client:row=>row.rut,responsible:row=>row.accountant,f29:row=>row.statusLabel,billing:row=>row.billingStatus,documents:row=>countFor(row),updated:row=>row.updated};return compareSortValues(values[sort.key](a),values[sort.key](b),sort.direction)||a.rut.localeCompare(b.rut,'es',{numeric:true});});
  const clearFilters=()=>{setQuery('');setResponsibleFilters([]);setDocumentFilters([]);setF29Filters([]);setBillingFilters([]);setOnlyAlerts(false);};
  const openDrive=(event:React.MouseEvent,row:ClientRow)=>{event.stopPropagation();if(row.driveFolderId)window.open(`https://drive.google.com/drive/folders/${row.driveFolderId}`,'_blank','noopener,noreferrer');};
  ...
  const inspector=inspectorId?rows.find(row=>row.id===inspectorId)??null:null;
  const activeFilters=responsibleFilters.length+documentFilters.length+f29Filters.length+billingFilters.length+Number(onlyAlerts);
  const sortClient=(key:ClientSortKey)=>setSort(current=>nextSort(current,key));
  return <div className={`clients-command-layout ${inspector?'has-inspector':''}`}><div className="control-content"><PageHeader eyebrow="Base maestra" title="Clientes" description="Cartera activa, cumplimiento y estado de facturación." actions={<button className="button-dark" onClick={()=>setEditing('new')}><Plus size={16}/> Nuevo cliente</button>}/>{error&&<div className="control-data-state is-error"><AlertTriangle size={16}/>{error}</div>}<section className="operations-card"><div className="table-toolbar clients-toolbar"><div className="toolbar-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar RUT, Conta o razón social…"/></div><button className={`clients-filter-toggle ${filtersOpen?'active':''}`} onClick={()=>setFiltersOpen(value=>!value)}><SlidersHorizontal size={14}/>Filtros{activeFilters>0&&<b>{activeFilters}</b>}</button><div className={`client-advanced-filters ${filtersOpen?'is-open':''}`}><MultiSelectFilter label="Responsables" allLabel="Todos los responsables" options={responsibles.map(name=>({value:name,label:name}))} values={responsibleFilters} onChange={setResponsibleFilters}/><MultiSelectFilter label="Estados F29" allLabel="Todos los estados F29" options={[{value:'missing',label:'Sin estado'},...Object.entries(F29_STATUS_LABELS).map(([value,label])=>({value,label}))]} values={f29Filters} onChange={setF29Filters}/><MultiSelectFilter label="Facturación" allLabel="Toda facturación" options={[{value:'not_applicable',label:'No aplica'},{value:'pending',label:'Pendiente'},{value:'sent',label:'Enviada'},{value:'paid',label:'Pagada'},{value:'overdue',label:'Vencida'}]} values={billingFilters} onChange={setBillingFilters}/><MultiSelectFilter label="Documentos" allLabel="Todos los documentos" options={[{value:'with',label:'Con documentos'},{value:'without',label:'Sin documentos'},{value:'unreviewed',label:'Sin revisar'}]} values={documentFilters} onChange={setDocumentFilters}/><button className={`filter-button ${onlyAlerts?'active':''}`} onClick={()=>setOnlyAlerts(value=>!value)}><AlertTriangle size={14}/>Alertas</button><button className="filter-button" onClick={clearFilters}>Limpiar</button></div></div><div className="table-scroll clients-command-scroll"><table className="ops-table clients-command-table"><thead><tr><SortableHeader label="Cliente / RUT" sort={sort} sortKey="client" onSort={sortClient}/><SortableHeader label="Responsable" sort={sort} sortKey="responsible" onSort={sortClient}/><SortableHeader label="F29" sort={sort} sortKey="f29" onSort={sortClient}/><SortableHeader label="Facturación" sort={sort} sortKey="billing" onSort={sortClient}/><SortableHeader label="Documentos" sort={sort} sortKey="documents" onSort={sortClient}/><SortableHeader label="Actualización" sort={sort} sortKey="updated" onSort={sortClient}/><th/></tr></thead><tbody>{filtered.map(row=><tr key={row.id} className={inspectorId===row.id?'is-selected':''} onClick={()=>setInspectorId(row.id)}><td><span className="command-client"><strong>{row.name}</strong><small>{row.rut}{row.accountingCode?` · Conta ${row.accountingCode}`:''}</small></span></td><td><span className={`assigned responsible-client ${color(row.accountant)}`}><EmptyAvatar initials={row.initials}/>{row.accountant}</span></td><td><Pill value={row.statusLabel}>{row.statusLabel}</Pill></td><td><StatusBadge status={row.billingStatus}/></td><td><strong>{countFor(row)} docs</strong></td><td>{row.updated}</td><td><button className="command-action" aria-label={`Inspeccionar ${row.name}`} onClick={event=>{event.stopPropagation();setInspectorId(row.id)}}><MoreHorizontal size={16}/></button></td></tr>)}</tbody></table></div><div className="clients-mobile-list">{filtered.map(row=><article key={`client-mobile-${row.id}`}><button onClick={()=>setInspectorId(row.id)}><span><strong>{row.name}</strong><small>{row.rut} · {row.accountant}</small></span><span><Pill value={row.statusLabel}>{row.statusLabel}</Pill><StatusBadge status={row.billingStatus}/></span><span><b>{countFor(row)} docs</b><em>{row.updated}</em></span></button></article>)}</div><footer className="table-footer"><span>{filtered.length} clientes</span><span>{rows.filter(row=>row.driveFolderId).length} con carpeta Drive</span></footer></section>{editing&&<ClientEditor client={editing==='new'?undefined:editing} onClose={()=>setEditing(null)} onSaved={async()=>{setEditing(null);await reload();}}/>}</div>{inspector&&<aside className="operation-inspector clients-inspector" aria-label={`Inspector de ${inspector.name}`}><header><div><span>Cliente</span><h2>{inspector.name}</h2><small>{inspector.rut}{inspector.accountingCode?` · Conta ${inspector.accountingCode}`:''}</small></div><button aria-label="Cerrar inspector" onClick={()=>setInspectorId(null)}><X size={18}/></button></header><div className="inspector-scroll"><section className="inspector-status"><Pill value={inspector.statusLabel}>{inspector.statusLabel}</Pill><StatusBadge status={inspector.billingStatus}/></section><section className="inspector-section"><h3>Resumen operativo</h3><dl><div><dt>Responsable</dt><dd>{inspector.accountant}</dd></div><div><dt>Documentos</dt><dd>{countFor(inspector)}</dd></div><div><dt>Monto F29</dt><dd>{inspector.amount===null?'—':new Intl.NumberFormat('es-CL',{style:'currency',currency:'CLP',maximumFractionDigits:0}).format(inspector.amount)}</dd></div><div><dt>Drive</dt><dd>{inspector.driveFolderId?'Conectado':'Sin carpeta'}</dd></div></dl></section>{inspector.observation&&<section className="inspector-alert"><AlertTriangle size={15}/><span><strong>Observación</strong>{inspector.observation}</span></section>}<section className="inspector-section"><h3>Acciones</h3><div className="inspector-actions"><button onClick={()=>go('client',inspector)}><ExternalLink size={15}/>Ficha completa</button><button onClick={()=>setEditing(inspector)}><Settings size={15}/>Editar cliente</button>{inspector.driveFolderId&&<button onClick={()=>window.open(`https://drive.google.com/drive/folders/${inspector.driveFolderId}`,'_blank','noopener,noreferrer')}><FolderOpen size={15}/>Abrir Drive</button>}</div></section><section className="inspector-section"><h3>Contacto</h3><dl><div><dt>Email</dt><dd>{inspector.legalRepresentativeEmail||'No informado'}</dd></div><div><dt>Teléfono</dt><dd>{inspector.phone||'No informado'}</dd></div><div><dt>Actualización</dt><dd>{inspector.updated}</dd></div></dl></section></div></aside>}</div>;
}
...
function ClientProfileV2({ client, year, month, reload, role, userId }: { client: ClientRow; year: number; month: number; reload: () => Promise<void>; role: 'admin'|'accountant'; userId?: string }) {
...
type TimelinePeriod = PeriodHistory & { upcoming?: boolean; placeholder?: boolean };
function withUpcoming(history: PeriodHistory[]): TimelinePeriod[] {
  const result = generateHistoryMonths(history).map(item => ({ amount: null, filed_date: null, status_code: null, status_label: null, ...item })) as TimelinePeriod[]; const now = new Date(); const existing = new Set(result.map(item=>`${item.year}-${item.month}`));
  for (let offset = 1; offset <= 2; offset++) { const date = new Date(now.getFullYear(), now.getMonth() + offset, 1); existing.add(`${date.getFullYear()}-${date.getMonth() + 1}`); }
...
function LegacyDocumentsPanelRecursive({ client, documents, refresh, readOnly = false }: { client: ClientRow; documents: ClientDocument[]; refresh: () => Promise<void>; readOnly?: boolean }) {
  const [scanning, setScanning] = useState(false);
  const [message, setMessage] = useState('');
  const [needsDriveAuthorization, setNeedsDriveAuthorization] = useState(false);
  const [query, setQuery] = useState('');
...
const DOCUMENT_GROUPS: DocumentGroup[] = ['F29 mensual', 'F22 / Renta', 'Comprobantes', 'Contratos / legales', 'Cobranza / pagos', 'Otros documentos'];
...
function DocumentsPanelRecursive({ client, documents, refresh, readOnly = false }: { client: ClientRow; documents: ClientDocument[]; refresh: () => Promise<void>; readOnly?: boolean }) {
  const [query,setQuery]=useState(''); const [type,setType]=useState<'all'|DocumentKind>('all'); const [year,setYear]=useState('all'); const [month,setMonth]=useState('all'); const [relevantOnly,setRelevantOnly]=useState(true); const [errorsOnly,setErrorsOnly]=useState(false); const [unclassifiedOnly,setUnclassifiedOnly]=useState(false); const [scanning,setScanning]=useState(false); const [message,setMessage]=useState('');
  const files=documents.filter(item=>!item.isFolder); const years=[...new Set(files.map(item=>item.modifiedAt?.slice(0,4)).filter(Boolean) as string[])].sort().reverse();
  const visible=files.filter(item=>{
    const date=item.modifiedAt?new Date(item.modifiedAt):null;
    return (!relevantOnly||isRecentRelevantDocument(item))&&(type==='all'||item.type===type)&&(year==='all'||date?.getFullYear()===Number(year))&&(month==='all'||(date ? date.getMonth()+1 : null)===Number(month))&&(!errorsOnly||item.processingStatus==='error')&&(!unclassifiedOnly||item.processingStatus==='unclassified')&&`${item.name} ${item.drivePath}`.toLowerCase().includes(query.toLowerCase());
  });
  const grouped=DOCUMENT_GROUPS.map(group=>({group,items:visible.filter(item=>documentGroup(item)===group)})).filter(section=>section.items.length);
  const scan=async()=>{if(readOnly)return;setScanning(true);setMessage('');try{const result=await scanClientDrive(client.id);setMessage(`${result.files_found} archivos encontrados · revisión actualizada.`);await refresh();}catch(error){setMessage(error instanceof Error?error.message:'No fue posible buscar en Drive.');}finally{setScanning(false);}};
  const classify=async(item:ClientDocument,next:DocumentKind)=>{if(readOnly)return;setMessage('Guardando clasificación…');try{await classifyDocument(item.id,next);await refresh();setMessage('Clasificación manual guardada y auditada.');}catch(error){setMessage(error instanceof Error?error.message:'No fue posible reclasificar.');}};
  return <section className="operations-card documents-panel profile-document-workspace"><div className="documents-head"><div><h2>Documentos operativos</h2><p>Archivos relevantes de los últimos 3 meses, agrupados por uso.</p>{message&&<small className="operation-message">{message}</small>}</div><div className="document-primary-actions"><button className="button-ghost" disabled={!client.driveFolderId} onClick={()=>client.driveFolderId&&window.open(`https://drive.google.com/drive/folders/${client.driveFolderId}`,'_blank','noopener,noreferrer')}><FolderOpen size={15}/>Buscar en Drive</button>{!readOnly&&<button className="button-dark" disabled={scanning||!client.driveFolderId} onClick={()=>void scan()}><RefreshCw size={15}/>{scanning?'Recargando…':'Recargar documentos'}</button>}</div></div><div className="document-filter-grid"><div className="toolbar-search"><Search size={15}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar archivo o ruta…"/></div><select value={type} onChange={event=>setType(event.target.value as 'all'|DocumentKind)}><option value="all">Todos los tipos</option>{(Object.keys(DOCUMENT_LABELS) as DocumentKind[]).map(value=><option value={value} key={value}>{DOCUMENT_LABELS[value]}</option>)}</select><select value={year} onChange={event=>setYear(event.target.value)}><option value="all">Todos los años</option>{years.map(value=><option key={value}>{value}</option>)}</select><select value={month} onChange={event=>setMonth(event.target.value)}><option value="all">Todos los meses</option>{MONTH_NAMES.map((name,index)=><option value={index+1} key={name}>{name}</option>)}</select><label><input type="checkbox" checked={relevantOnly} onChange={event=>setRelevantOnly(event.target.checked)}/>Relevantes · 3 meses</label><label><input type="checkbox" checked={errorsOnly} onChange={event=>setErrorsOnly(event.target.checked)}/>Con errores</label><label><input type="checkbox" checked={unclassifiedOnly} onChange={event=>setUnclassifiedOnly(event.target.checked)}/>Sin clasificar</label></div><div className="document-groups">{grouped.map(section=><section key={section.group}><header><h3>{section.group}</h3><span>{section.items.length}</span></header><div className="document-compact-list">{section.items.map(item=><article key={item.id}><div className="document-card-main"><FileSpreadsheet size={17}/><span><strong title={item.name}>{item.name}</strong><small>{DOCUMENT_LABELS[item.type]} · {item.module==='f29'?'F29':item.module==='f22'?'F22':'Cliente'} · Drive</small><small>{item.modifiedAt?`${MONTH_NAMES[new Date(item.modifiedAt).getMonth()]} ${new Date(item.modifiedAt).getFullYear()}`:'Período no detectado'} · Indexado {item.indexedAt?dateTime(item.indexedAt):'sin fecha'}</small></span>{item.classificationSource==='manual'&&<em>Manual</em>}</div><div className="document-card-actions">{item.driveUrl&&<a href={item.driveUrl} target="_blank" rel="noreferrer">Abrir <ExternalLink size={12}/></a>}{!readOnly&&<select aria-label={`Reclasificar ${item.name}`} value={item.type} onChange={event=>void classify(item,event.target.value as DocumentKind)}>{(Object.keys(DOCUMENT_LABELS) as DocumentKind[]).map(value=><option value={value} key={value}>{DOCUMENT_LABELS[value]}</option>)}</select>}</div></article>)}</div></section>)}</div>{!grouped.length&&<div className="document-empty-state"><FolderOpen size={24}/><strong>No hay documentos relevantes para este período.</strong><p>Recarga los documentos del cliente o amplía los filtros para revisar el archivo completo.</p>{!readOnly&&<button disabled={!client.driveFolderId||scanning} onClick={()=>void scan()}>Recargar documentos del cliente</button>}</div>}</section>;
}
