...
export const stripDuplicateClosing = (html: string) => html
  .replace(/<p[^>]*>\s*Saludos cordiales[\s\S]*?<\/p>\s*$/i, '')
  .replace(/Saludos cordiales\s*(?:<br\s*\/?>\s*)?(?:Netz Asesor[ií]as|\{\{firm_name\}\})?\s*$/i, '')
  .trim();

export function renderNetzEmail(input: NetzEmailTemplateInput) {
  const bodyHtml = stripDuplicateClosing(input.bodyHtml);
